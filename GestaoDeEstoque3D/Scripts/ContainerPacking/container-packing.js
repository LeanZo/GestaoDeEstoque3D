...
var ViewModel = function () {
	...
	self.ShowPackingView = function (containerPackingResult) {
		var algorithmPackingResult = containerPackingResult.AlgorithmPackingResults
	...
	self.PackItemInRender = function () {
		if (itemIndex < ContainerPackingResult[0].AlgorithmPackingResults[0].PackedItems.length - 1) {
			itemIndex++;

			var PackedItems = ContainerPackingResult[0].AlgorithmPackingResults[0].PackedItems;

			var itemOriginOffset = {
				x: PackedItems[itemIndex].PackDimX / 2,
				y: PackedItems[itemIndex].PackDimY / 2,
				z: PackedItems[itemIndex].PackDimZ / 2
			};

			var itemGeometry = new THREE.BoxGeometry(PackedItems[itemIndex].PackDimX, PackedItems[itemIndex].PackDimY, PackedItems[itemIndex].PackDimZ);
			var cube = new THREE.Mesh(itemGeometry, itemMaterial);
			cube.position.set(self.ContainerOriginOffset.x + itemOriginOffset.x + PackedItems[itemIndex].CoordX, self.ContainerOriginOffset.y + itemOriginOffset.y + PackedItems[itemIndex].CoordY, self.ContainerOriginOffset.z + itemOriginOffset.z + PackedItems[itemIndex].CoordZ);
	...
	self.UnpackItemInRender = function () {
		if (itemIndex > -1) {
			var selectedObject = scene.getObjectByName('cube' + itemIndex);
			scene.remove( selectedObject );
			itemIndex--;
        }
	};
